...
class Block(StartPolicyInterface):
    ...
    def validBlocks(self, task, dbs):
        ...
        for blockName in blocks:
            ...
            if task.getLumiMask():
                ...
                #ratio = total num lumis / lumis which are ok in the block
                ratio_accepted = 1. * sum( [ len(maskedBlocks[blockName][file]) for file in maskedBlocks[blockName] ] ) / \
                                        float(block['NumberOfLumis'])
    ...
    def getMaskedBlocks(self, task, dbs, datasetPath):
        """ Get the blocks which pass the lumi mask restrictions. For each block return the list of lumis
            which were ok g(iven the lumi mask). The data structure returned is the following:
